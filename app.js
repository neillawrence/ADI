...
function setupAuthGate() {
  const accountKey = 'roadwiseLocalAccount';
  const sessionKey = 'roadwiseAuthenticated';
  const account = () => { try { return JSON.parse(localStorage.getItem(accountKey)); } catch (error) { return null; } };
  const gate = document.createElement('main');
  gate.className = 'auth-gate';
  gate.innerHTML = '<section class="auth-card"><div class="brand auth-brand"><span class="brand-mark">R</span><span>roadwise</span></div><p class="eyebrow">Private ADI studio</p><h1 id="authTitle">Sign in to your studio</h1><p class="auth-copy" id="authCopy">Use your account details to continue.</p><form id="authForm"><label>Email<input name="email" type="email" autocomplete="email" required placeholder="you@example.com"></label><label>Password<input name="password" type="password" autocomplete="current-password" required minlength="8" placeholder="At least 8 characters"></label><button class="button button-dark" type="submit" id="authSubmit">Sign in</button><p class="auth-error" id="authError" role="alert"></p></form></section>';
  document.body.prepend(gate);
  const form = $('#authForm');
  const error = $('#authError');
  const refreshMode = () => { const hasAccount = Boolean(account()); $('#authTitle').textContent = hasAccount ? 'Sign in to your studio' : 'Create your studio account'; $('#authCopy').textContent = hasAccount ? 'Use your account details to continue.' : 'Set a private account for this browser before continuing.'; $('#authSubmit').textContent = hasAccount ? 'Sign in' : 'Create account'; form.querySelector('input[name="password"]').autocomplete = hasAccount ? 'current-password' : 'new-password'; };
  const unlock = () => { gate.hidden = true; document.body.classList.remove('auth-locked'); };
  const lock = () => { sessionStorage.removeItem(sessionKey); gate.hidden = false; document.body.classList.add('auth-locked'); refreshMode(); form.reset(); };
  form.addEventListener('submit', async (event) => { event.preventDefault(); error.textContent = ''; const data = new FormData(form); const email = data.get('email').trim().toLowerCase(); const password = data.get('password'); const passwordHash = await hashPassword(password); const existing = account(); if (existing && (existing.email !== email || existing.passwordHash !== passwordHash)) { error.textContent = 'Those account details could not be verified.'; return; } localStorage.setItem(accountKey, JSON.stringify({ email, passwordHash })); sessionStorage.setItem(sessionKey, 'authenticated'); unlock(); toast(existing ? 'Signed in to your studio.' : 'Your private studio account is ready.'); });
  $('.top-avatar').title = 'Sign out';
  $('.top-avatar').addEventListener('click', lock);
  refreshMode();
  if (sessionStorage.getItem(sessionKey) === 'authenticated' && account()) unlock(); else { document.body.classList.add('auth-locked'); gate.hidden = false; }
}
